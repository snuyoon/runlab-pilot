...
- **같은 세션 내 중복 보고 방지(2026-07-04)**: 이미 보고한 항목은 **목록에서 숨기지 않고 회색 비활성 + "이번 주 완료" 배지**로 표시(선택 불가) — 참여자가 "왜 사라졌지?" 혼란 없이 이미 응답했음을 인지. ① 정신 건강은 주간 배터리라 1회 보고 후 Q5에서 비활성. ② 부상/질병은 복수 허용(무릎+발목 등 — 원판의 문제 단위 반복 유지)하되 이미 보고한 부위(Q6)/증상군(Q7)만 비활성(진짜 중복 차단). ③ 반복 연결 후보(prev)는 이번 세션에 쓴 루트를 목록에서 제외.
- **탈출구**: 유형 선택 페이지(Q5)에 **"해당 없음 — 지금까지 응답 제출(N건)" 버튼** 상시 노출 — 잘못 진입했거나 '예(또 있음)'를 오탭한 참여자가 부상/질병/정신 3개 유형이 보이는 화면에서 바로 빠져나옴. 완료 문제가 없으면 "해당 없음 — 설문 종료"로 표기.
